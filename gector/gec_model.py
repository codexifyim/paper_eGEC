...
logging.getLogger("egector").setLevel(logging.ERROR)
...
class GecBERTModel(object):
    ...
    def get_token_action(self, token, index, prob, sugg_token):
        ...
        if sugg_token.startswith('$REPLACE_') or sugg_token.startswith('$TRANSFORM_') or sugg_token.startswith('$SPELLCORRECT_') or sugg_token.startswith('$INFLECT_') or sugg_token == '$DELETE' :
            start_pos = index
            end_pos = index + 1
